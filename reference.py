...
classifiers = {
    "CWS-ENC": mt.CWS_ENC(method=0, metric=distance.euclidean, random_state=rs),
...
for i, dataset in enumerate(tqdm(datasets, desc="DBS", ascii=True)):
    ...
    for fold, (train, test) in enumerate(
        tqdm(skf.split(X, y), desc="FLD", ascii=True, total=5, disable=True)
    ):
        X_train, X_test = X[train], X[test]
        y_train, y_test = y[train], y[test]
        for c, clf_name in enumerate(tqdm(classifiers, desc="CLF", ascii=True, disable=True)):
            clf = base.clone(classifiers[clf_name])
            clf.fit(X_train, y_train)
            y_pred = clf.predict(X_test)

            for m, metric_name in enumerate(tqdm(used_metrics, desc="MET", ascii=True, disable=True)):
                try:
